...
**다음 우선순위**
1. **jutaku・kaigo에서 다룬 도쿄 5개 구를 shussan과 맞춰, 이 5개 구를 다른 카테고리(sogyo・energy)에도 확장할지 검토** — 지금까지 shussan・jutaku・kaigo 3개 카테고리가 전부 같은 5개 구(千代田・港・品川・世田谷・渋谷) 세트로 데이터가 쌓였다. sogyo・energy는 아직 국가단위 1건씩만 있음 — 이 2개 카테고리도 같은 5개 구로 지자체 독자제도를 조사하면(예: 창업지원금・에너지설비조성 등 구 독자분), 5개 카테고리 전부 5개 구 비교 데이터가 갖춰져 特集 콘텐츠 설계가 쉬워진다.
...
3. **jGrants 기반 sogyo・energy 확충은 후순위** — 이번 조사로 jGrants 공개목록에 개인대상 신규 전국단위 제도가 당장 없음을 확인했다. 정기적으로(예: 월1회) `npm run subsidies`로 신규 공모만 재확인하고, 무리하게 사업자용/지자체한정 제도를 끼워넣지 말 것.
4. **`shussan`・`jutaku`・`kaigo` 카테고리 지자체 데이터가 더 쌓이면 첫 特集 기사 작성 검토** — 3개 카테고리 모두 5개 구 비교 데이터가 있지만, 「TOP5」류 순위 콘텐츠를 만들려면 순위를 매길 만한 차별화 포인트를 먼저 편집 기준으로 정해야 함. jutaku・kaigo는 제도 성격이 갈리므로(위 참조), 카테고리를 나눠서 특집을 설계하는 편이 자연스러울 수 있음.
