...
**다음 우선순위**
1. **5개 카테고리 전부 5개 구 데이터가 쌓였으니, 첫 特集(tokushu) 기사 작성 착수 검토** — 예를들어 "창업 자기부담이 가장 낮은 구 TOP5"(sogyo, 융자알선4구만 비교 — 港区는 현금보조형이라 별도 취급 필요), "요개호 인정 없이 이용할 수 있는 고령자 지원 TOP"(kaigo) 등, 카테고리별로 순위를 매길 만한 명확한 차별화 축을 먼저 편집기준으로 정할 것. jutaku・kaigo・energy・sogyo는 전부 제도유형이 갈리므로(위 참조), 순위 비교시 같은 유형끼리만 묶어야 함.
2. **`src/config/regions.ts`에 도쿄 23구 잔여 18개 구 및 타 도도부현 시구정촌 추가** — regions.ts 자체 주석에 "기사화・데이터 확보가 진행된 지역부터 추가"라는 원칙이 있으므로, 기사 없이 지자체만 먼저 등록하지 말 것. 새 지자체로 기사를 쓰기로 확정한 뒤에 등록하는 순서를 지킬 것.
3. **jGrants 기반 sogyo・energy 확충은 후순위** — 조사로 jGrants 공개목록에 개인대상 신규 전국단위 제도가 당장 없음을 확인했다. 정기적으로(예: 월1회) `npm run subsidies`로 신규 공모만 재확인하고, 무리하게 사업자용/지자체한정 제도를 끼워넣지 말 것.
