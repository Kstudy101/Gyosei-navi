/**
 * sourceLinks URL 생존 확인 (一次情報 링크 절단 감시) v2
...
function collectTargets(): Target[] {
  ...
  for (const file of walk(CONTENT_DIR)) {
    ...
    for (const link of data.sourceLinks ?? []) {
      targets.push({ file: rel, status: data.status ?? "draft", label: link.label, url: link.url });
    }
    if (data.subsidy?.applyUrl) {
      targets.push({
        file: rel,
        status: data.status ?? "draft",
        label: "subsidy.applyUrl",
        url: data.subsidy.applyUrl,
      });
    }
  }
  return targets;
...
async function main(): Promise<void> {
  ...
  console.log(
    `출처 링크 ${targets.length}건 / 고유 URL ${uniqueUrls.length}건을 확인합니다 (같은 호스트는 1초 간격)…\n`
  );
  ...
  console.log(
    `\n확인 완료: 출처 링크 ${targets.length}건 / 고유 URL ${uniqueUrls.length}건 / published 기사의 죽은 링크 ${publishedDead.length}건`
  );
