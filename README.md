...
- **`energy`・`sogyo` 카테고리에도 도쿄 5개 구 지자체 비교 완료 — 5개 카테고리 전부 5개 구 세트 완비.** energy는 千代田区(EV充電設備・V2H)・港区/品川区/渋谷区(エネファーム, 산정방식은 정률÷4/정액20万/정액30万으로 갈림)・世田谷区(재에너지 전력계약전환)로 4패턴. **世田谷区만 구민이 구에 직접 신청하는 구조가 아니라 전기회사가 캠페인을 운영하고 환원금을 지급**한다는 구조적 차이가 있어, 사용자 확인 후 포함하되 기사・비교기사 양쪽에 명시했다. 品川区는 우산형 제도(しながわゼロカーボンアクション助成) 중 エネファーム 메뉴만 접수여유이고 太陽光発電등 타 메뉴는 予定件数超過임을 원문 표에서 확인해 기사에 반영. sogyo는 千代田・品川・世田谷・渋谷(융자알선형, 구가 이자를 보급해 본인부담률0.1〜0.4%) vs 港区(**현금 직접보조형**, 대상경비3분의2・최대250万円)로 갈려 역시 제도유형 정리로 비교. 品川区 창업지원금은 에이전트 보고(이자보급1.6%)와 PDF원문(실제1.4%) 사이에 세부오차가 있어 원문 재확인으로 정정. **이 두 카테고리부터 "기사 1건→검증→커밋→푸시→CI/Deploy 완료확인"을 기사 단위로 반복하는 방식**으로 전환(여러 기사를 한 커밋에 몰아넣지 않음).
- 웹서버(Xserver) 배포 직후 서버 캐시(10분)로 구버전이 일시적으로 보일 수 있음을 확인 — 재배포 전에 캐시 우회 요청으로 먼저 재확인할 것.
- `npm run build` 정상 통과, published 기사 35건(5개 카테고리 각 1건 + shussan・jutaku・kaigo・energy・sogyo 지자체 각 5건 + compare 5건) 모두 정적 생성・검색 색인됨을 확인.
...
**다음 단계**:
- 5개 카테고리 전부 5개 구 데이터가 쌓였으니, 첫 特集(순위) 기사 작성 착수 검토. jutaku・kaigo・energy・sogyo는 전부 제도유형이 갈리므로 같은 유형끼리만 순위비교할 것.
...
> `npm run build`는 정적 export(out/)로 정상 생성된다. published 기사는 35건(5개 카테고리 Pillar 각 1건 + shussan・jutaku・kaigo・energy・sogyo 지자체 각 5건 + 지역비교 5건).
