...
- **`jutaku`(주택리폼) 카테고리에도 도쿄 5개 구 지자체 비교 완료** — shussan과 같은 「원문조사→개별기사→compare기사」 패턴을 적용했으나, 5개 구의 제도가 금액서열로 비교할 수 없는 4가지 성격(省エネ改修型: 千代田・世田谷 / 窓特化型: 港 / 複合型: 品川 / 耐震型: 渋谷)으로 갈리는 것을 확인해, 비교 기사는 금액 순위가 아니라 「제도 유형 정리」로 설계했다. 원문 재확인 과정에서 千代田区가 予算到達で受付を一旦終了 상태임을 발견(최초 조사 에이전트 보고에는 누락돼 있었음) — 서브에이전트 보고를 그대로 믿지 말고 원문으로 재검증해야 한다는 사례.
- **`kaigo`(개호) 카테고리에도 도쿄 5개 구 지자체 비교 완료** — jGrants API로 kaigo・energy에 쓸 전국단위 개인대상 제도를 먼저 찾아봤으나(382건 중 kaigo 7건・energy 16건), 전부 사업자용・지자체 한정이라 채택하지 않고 지자체비교 패턴으로 확장. 5개 구가 現物支給型(千代田: 紙おむつ)・利用者負担軽減型(港・渋谷)・見守り支援型(品川: 요개호 불문)・在宅介護慰労型(世田谷: 연액10만엔)의 4패턴으로 갈려, 이번에도 금액순위가 아닌 「제도 유형 정리」로 비교 기사를 설계했다. 원문 5건 전량을 재취득해 대조한 결과 이번에는 에이전트 보고 누락 없음을 확인.
- 웹서버(Xserver) 배포 직후 서버 캐시(10분)로 구버전이 일시적으로 보일 수 있음을 확인 — 재배포 전에 캐시 우회 요청으로 먼저 재확인할 것.
- `npm run build` 정상 통과, published 기사 23건(5개 카테고리 각 1건 + shussan・jutaku・kaigo 지자체 각 5건 + compare 3건) 모두 정적 생성・검색 색인됨을 확인.
- **(참考) 이 프로젝트는 여러 세션이 동시에 작업할 수 있다.** 楽天アフィリエイト 연동(`src/lib/ads/`, `src/components/ads/`)은 별도 세션이 병행 작업한 것으로, v2 전환 작업과는 별개 계통이다. 세션별 작업 완료 기록은 `workspace/<날짜>-<주제>/`(gitignore 대상)에 남기는 관례가 있다. **커밋 시 `git add`는 반드시 파일을 명시적으로 나열할 것** — 작업 중 리포 루트에서 실제 SSH 배포키(다른 세션 소관)가 발견된 적이 있어, `-A`/`.`로 통째로 add하면 의도치 않게 시크릿을 커밋할 위험이 있다.

**다음 단계**:
- shussan・jutaku・kaigo에서 이미 쌓인 도쿄 5개 구 세트를 sogyo・energy로도 확장할지 검토(전 카테고리를 같은 5개 구로 맞추면 特集 설계가 쉬워짐).
- `src/config/regions.ts`에 도쿄 23구 잔여 18개 구 및 타 도도부현 데이터 추가 — 단, "기사화가 확정된 지역부터 등록"하는 기존 원칙을 지킬 것(코드에 지자체만 먼저 채우지 않는다).
- jGrants 기반 sogyo・energy 확충은 후순위 — 조사 결과 지금은 개인대상 신규 전국단위 제도가 없음을 확인했다. 정기적으로 신규 공모만 재확인.
- `shussan`・`jutaku`・`kaigo`의 지자체 데이터가 더 쌓이면 첫 特集(순위) 기사 작성 검토. jutaku・kaigo는 제도 성격이 갈리므로 카테고리를 나눠 설계하는 편이 나을 수 있음.
...
> `npm run build`는 정적 export(out/)로 정상 생성된다. published 기사는 23건(5개 카테고리 Pillar 각 1건 + shussan・jutaku・kaigo 지자체 각 5건 + 지역비교 3건).
