...
- `npm run build` 정상 통과, published 기사 17건(5개 카테고리 각 1건 + shussan 지자체 5건 + jutaku 지자체 5건 + compare 2건) 모두 정적 생성・검색 색인됨을 확인.
...
- `sogyo`・`energy`・`kaigo` 카테고리는 jGrants 등에서 카테고리당 Cluster 기사 추가(지자체 비교보다 국가단위 제도 확충이 자연스러움).
- `shussan`・`jutaku`의 지자체 데이터가 더 쌓이면 첫 特集(순위) 기사 작성 검토. jutaku는 제도 성격이 갈리므로 카테고리를 나눠 설계하는 편이 나을 수 있음.
...
> `npm run build`는 정적 export(out/)로 정상 생성된다. published 기사는 17건(5개 카테고리 Pillar 각 1건 + shussan 지자체 5건 + jutaku 지자체 5건 + 지역비교 2건).
