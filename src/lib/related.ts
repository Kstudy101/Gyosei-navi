 ...
 *   2. 同カテゴリ + タグ重複数の多い順
 */
export function getRelatedArticles(article: Article, all: Article[], limit = 4): Article[] {
  ...
  const scored = others
    ...
    .map((a) => {
      ...
      const sameCategory = a.frontmatter.category === article.frontmatter.category ? 1 : 0;
