...
> ⚠️ **정지 및 폐기 대상 (2026-09-20).** 「전국 보조금 정보 사이트」 전환에 따라 본 파이프라인이 다루던 행정서사 카테고리 로테이션은 무효. **작업 스케줄러에 등록된 자동 실행이 있다면 지금 바로 중단할 것** — 존재하지 않는 카테고리를 대상으로 헤드리스 세션이 계속 실행되면 오류가 반복된다. 재구축 여부는 코드 재작성 세션 이후 별도 결정(`docs/04_EDITORIAL_GUIDELINE.md` v2 §7 참조).
