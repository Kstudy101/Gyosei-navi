...
> ⚠️ **v1 유산 문서 (2026-09-20).** 본 프로젝트는 「행정서사 종합미디어」에서 「전국 보조금 정보 사이트」로 전면 전환됐다(`docs/00_MASTER_PLAN.md` v2 참조). `src/lib/sources/jgrants.ts`・`http.ts` 등 일부 코드 자산은 재활용되지만, 본 문서의 작업지시 자체는 v1 도메인 기준이라 새 작업의 근거로 쓰지 말 것.
